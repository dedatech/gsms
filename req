...
- **功能描述**：系统后台管理功能，确保系统有序运行。
    ...
    2.  **全局权限管理**：
        -   定义系统级角色（如：系统管理员、部门经理、普通员工），并配置其访问各功能模块的权限。
        -   此处的权限是项目级权限的基础。
...
必须按照下面要求顺序执行：
- **框架搭建：**
 1、框架要具备比较高的抽象，方便代码为维护，比如：通用的常量类、枚举类、通用方法和工具、统一的请求和返回报文封装，统一的错误code定义，统一的身份校验解决方案，要采用token模式
...
- **需求分析：**
 ...
 2、数据库设计，讲数据库设计的ddl，项目工程的指定目录中，供我审核
