  ...
  <div class="attachment-list">
    ...
    <div class="list-body">
      <el-table
        ...
        empty-text="暂无附件"
      >
        <el-table-column type="selection" width="55" v-if="canDelete" />
...
const canDelete = computed(() => {
  return attachments.value.some(att => canEdit(att))
})
...
const fetchData = async () => {
  loading.value = true
  try {
    const list = await getAttachmentList(props.targetType, props.targetId)
...
const handleSelectionChange = (selection: AttachmentInfo[]) => {
...
const handleFileChange = async (event: Event) => {
  ...
  try {
    await uploadAttachment(props.targetType, props.targetId, file)
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedAttachments.value.length} 个附件吗？`,
      '批量删除',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    const ids = selectedAttachments.value
      .filter(att => canEdit(att))
      .map(att => att.id)

    if (ids.length === 0) {
      ElMessage.warning('您没有权限删除选中的附件')
      return
    }
