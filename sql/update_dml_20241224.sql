-- ============================================
-- 文件名：update_dml_20241224.sql
-- 说明：初始化角色、权限点及角色-权限关系
-- 注意：
--   1）假设 role / permission / role_permission 目前为空；
--   2）如果已有数据，请根据实际情况调整 id 取值，避免主键冲突。
-- ============================================

-- 1. 预置系统角色（role）
INSERT INTO role (id, name, code, description, role_level, create_time, update_time)
VALUES 
...
-- 2. 预置权限点（permission）
INSERT INTO permission (id, name, code, description, create_time, update_time)
VALUES
...
-- 4. 示例：给具体用户绑定角色（按需启用）
-- INSERT INTO user_role (id, user_id, role_id, create_user_id, create_time)
-- VALUES
